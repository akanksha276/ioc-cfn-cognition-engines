...
│   ├── agent/
│   │   ├── semantic_negotiation.py      # Top-level pipeline orchestrator
...
Create `semantic_negotiation/.env`:
...
```bash
# Unit tests
PYTHONPATH=semantic_negotiation poetry run pytest semantic_negotiation/tests/unit -v

# Offer validation tests (fuzzy + embedding tiers)
PYTHONPATH=semantic_negotiation poetry run pytest semantic_negotiation/tests/unit/test_offer_validation.py -v
```
