...
Run as a module (recommended) from ``ioc-cfn-cognitive-agents``::
    ...
    PYTHONPATH=semantic_alignment poetry run python -m app.agent.options_generation

Or after activating the venv, from ``semantic_alignment``::

    PYTHONPATH=. python -m app.agent.options_generation

Direct ``python path/to/options_generation.py`` is supported: the repo root
``semantic_alignment/`` is prepended to ``sys.path`` so ``app.*`` imports resolve.
...
# ``.../semantic_alignment/app/agent/this_file.py`` → parent of package ``app``
_semantic_alignment_root = Path(__file__).resolve().parents[2]
if str(_semantic_alignment_root) not in sys.path:
    sys.path.insert(0, str(_semantic_alignment_root))
