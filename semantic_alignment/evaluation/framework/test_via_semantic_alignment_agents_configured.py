...
"""test_via_semantic_alignment_agents_configured.py

LLM negotiation agents with per-mission persona configs loaded from
``datasets/agent_configs.yaml``.

This is the **configured variant** of ``test_via_semantic_alignment_agents.py``.
...
::
    ...
    cd semantic_alignment
    poetry run python evaluation/framework/test_via_semantic_alignment_agents_configured.py \
        --filter hard
...
# File lives at semantic_alignment/evaluation/framework/ — 3 levels inside the repo root.
_workspace_root = str(Path(__file__).resolve().parent.parent.parent.parent)
if _workspace_root not in sys.path:
    sys.path.insert(0, _workspace_root)

# Add semantic_alignment/app so config.utils is importable
