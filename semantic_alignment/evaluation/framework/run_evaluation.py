...
Usage::
    ...
    # From the repo root:
    .venv/bin/python semantic_alignment/evaluation/framework/run_evaluation.py
...
# ── sys.path: make semantic_alignment + app importable ────────────────────
for _p in (
    str(Path(__file__).resolve().parents[3]),  # repo root
    str(Path(__file__).resolve().parents[3] / "semantic_alignment" / "app"),
):
...
def main(config_path: Path, n_missions: int | None = None) -> None:
    ...
    if config.mechanism == "callback":
        from semantic_alignment.evaluation.framework.mechanisms.callback_env import (  # noqa: E402
            CallbackEnvRunner,
        )
        ...
        from semantic_alignment.evaluation.framework.runner import (  # noqa: E402
            EvaluationRunner,
        )
