...
"""Sample environment — Callback mode (mirrors test_via_semantic_alignment_agents.py).

This sample drives negotiations through the **external negotiation server** at
port 8089, exactly like ``test_via_semantic_alignment_agents.py``.  The agent logic runs
...
Usage::
    ...
    .venv/bin/python -m semantic_alignment.evaluation.framework.mechanisms.callback_env
...
_sn_root = str(Path(__file__).resolve().parents[3])  # semantic_alignment/
...
from semantic_alignment.evaluation.framework.config import (  # noqa: E402
    AgentConfig,
    EvaluationConfig,
    MissionEntry,
)
...
# ─────────────────────────────────────────────────────────────────────────────
# SSTP message helpers (identical pattern to test_via_semantic_alignment_agents.py)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Agent FastAPI server  (same /decide pattern as test_via_semantic_alignment_agents.py)
# ─────────────────────────────────────────────────────────────────────────────
...
def _make_agent_app(registry: Dict[str, _CallbackAgent]) -> FastAPI:
    ...
    async def decide(request: Request) -> JSONResponse:
        ...
        def _one(body: Dict[str, Any]) -> Dict[str, Any]:
            ...
            # can build replies_by_pid correctly (mirrors test_via_semantic_alignment_agents.py).
        ...
        # Run all decisions in parallel threads (matches test_via_semantic_alignment_agents.py)
...
# ─────────────────────────────────────────────────────────────────────────────
# CallbackEnvRunner — mirrors test_via_semantic_alignment_agents.py
# ─────────────────────────────────────────────────────────────────────────────
...
class CallbackEnvRunner:
    ...
    This runner mirrors ``test_via_semantic_alignment_agents.py`` exactly:
    ...
    def run(self) -> CallbackEvalResult:
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "Start it with:\n"
                "  cd semantic_alignment && poetry run uvicorn app.main:app "
