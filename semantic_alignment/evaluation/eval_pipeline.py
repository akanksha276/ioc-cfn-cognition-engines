...
``test_via_semantic_alignment_agents_configured.py``. No additional LLM calls.
...
# ── sys.path: repo root + semantic_alignment/app must be importable ────────
_repo_root = str(Path(__file__).resolve().parents[2])  # ioc-cfn-cognitive-agents/
_sn_root = str(Path(__file__).resolve().parents[1])    # semantic_alignment/
_sn_app = str(Path(__file__).resolve().parents[1] / "app")  # semantic_alignment/app
for _p in (_repo_root, _sn_root, _sn_app):
    if _p not in sys.path:
        sys.path.insert(0, _p)

# Load .env from semantic_alignment/ before importing settings/litellm
...
def load_trace_dir(trace_dir: str) -> Dict[str, Dict[str, Any]]:
    """Scan a ``test_via_semantic_alignment_agents.py`` run directory for initiate responses.
...
def _entry_to_slug(entry_id: str) -> str:
    ...
    format used by ``test_via_semantic_alignment_agents.py``.
...
def load_run_log(path: str) -> Dict[str, Any]:
    """Load a ``run_log.json`` produced by ``test_via_semantic_alignment_agents*.py``.
...
def run_evaluation(
    ...
    *,
    ...
    Args:
        ...
        trace_dir: Path to a ``test_via_semantic_alignment_agents.py`` run directory
        ...
        run_log_path: Path to a ``run_log.json`` produced by
            ``test_via_semantic_alignment_agents_configured.py``.  When provided,
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    parser.add_argument(
        ...
        metavar="DIR",
        help=(
            "Offline mode: path to a test_via_semantic_alignment_agents.py run directory "
    ...
    parser.add_argument(
        ...
        metavar="FILE",
        help=(
            "Path to run_log.json from test_via_semantic_alignment_agents_configured.py. "
