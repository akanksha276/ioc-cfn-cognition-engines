...
"""Async top-k retrieval against the RAG /rag/similarity-search API (CoDi-local)."""
...
async def retrieve_rag_top_k(
    ...
    """
    Run similarity via POST /rag/similarity-search on ``repo`` (object with ``search_similar_rag``).
    ...
    try:
        ...
        logger.warning("[CoDi RAG] /rag/similarity-search failed: %s", e)
        return []
    ...
    for i, r in enumerate(raw or [], start=1):
        ...
        if n:
            ...
            out.append(n)
    logger.info("[CoDi RAG] retrieval result count=%d", len(out))
    return out
